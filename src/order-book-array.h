...
namespace OrderBookProgrammingProblem {
  ...
  template<typename T>
  concept AllowedPriceLevelImpl = std::same_as<T, PriceLevelArray> ||
                                  std::same_as<T, PriceLevelDict>;
  ...
  class OrderBookArray : public IOrderBook<OrderBookArray> {
    ...
    get_pricer_cost_cent(std::vector<T> &prices, const View &transform,
                         int target_size) {
      int cost_cent = 0;
      // This loop is hugely inefficient, say there is only one bid price at 40 but size smaller than target_size, then it iterates from 4000 to 0
      for (auto &price_level: transform(prices)) {
        ...
        if (!level_price_cent.has_value()) {
          ...
          if constexpr (!benchmark_performance)
            std::cerr << "prices.size(): " << prices.size() << "hit: " << hit << ", miss: " << miss << ", hit rate: " <<
                hit / (hit + miss) << "\n";
          continue;
        }
    ...
    std::string get_repr_of_one_side(std::vector<PriceLevelImpl> &price_levels,
  ...
  public:
    ...
    std::optional<int> FUNC_ATTRIBUTE get_pricer_sell_cost_cent_impl(int target_size) {
      int cost_cent = 0;
      if (bid_prices.empty()) return std::nullopt;
      for (auto i = bid_prices.size() - 1; i >= lowest_bidding_cent; --i) {
        ...
        if (!level_price_cent.has_value()) {
          if constexpr (!benchmark_performance) {
            ++bid_miss;
            std::cerr << "[bid_miss] lowest_bidding_cent: " << lowest_bidding_cent << ", i: " << i << ", hit: " <<
                bid_hit << ", miss: " << bid_miss << ", hit rate: " << bid_hit / (bid_hit + bid_miss) << "\n";
          }
          continue;
        }
        auto level_size = price_level.get_level_size();
        ...
        if constexpr (!benchmark_performance) {
          ++bid_hit;
          std::cerr << "[bid_hit] lowest_bidding_cent: " << lowest_bidding_cent << ", i: " << i << ", hit: " << bid_hit
              << ", miss: " << bid_miss << ", hit rate: " << bid_hit / (bid_hit + bid_miss) << "\n";
        }
      ...
      */
      ...
      for (ssize_t i = lowest_asking_cent; i < ask_prices.size(); ++i) {
        ...
        if (!level_price_cent.has_value()) {
          ++ask_miss;
          if constexpr (!benchmark_performance)
            std::cerr << "[ask_miss] lowest_asking_cent: " << lowest_asking_cent << ", i: "
                << i << ", ask_prices.size(): " << ask_prices.size() << ", hit: " << ask_hit << ", miss: " <<
                ask_miss << ", hit rate: " << ask_hit / (ask_hit + ask_miss) << "\n";
        ...
        ++ask_hit;
        if constexpr (!benchmark_performance)
          std::cerr << "[ask_hit] lowest_asking_cent: " << lowest_asking_cent << ", i: "
              << i << ", ask_prices.size(): " << ask_prices.size() << ", hit: " << ask_hit << ", miss: " <<
              ask_miss << ", hit rate: " << ask_hit / (ask_hit + ask_miss) << "\n";
    ...
    void add_order_impl(const Order::LimitOrder &new_order) {
      ...
      if (new_order.type == Order::Type::Add) {
        ...
        if (new_order.side == Order::Side::Bid && new_order.price_cent < lowest_bidding_cent) {
          lowest_bidding_cent = new_order.price_cent;
        }
        if (new_order.side == Order::Side::Ask && new_order.price_cent < lowest_asking_cent) {
      ...
      auto update_order = [&](auto &prices) {
        auto new_price_level_size = prices[existing_order->price_cent].update_order(order_ptr);
        if (new_price_level_size <= 0) {
          ...
          if (existing_order->side == Order::Side::Ask) {
            ...
            while (!ask_prices.empty()) {
              if (!ask_prices[ask_prices.size() - 1].get_level_size_impl().has_value())
                ask_prices.pop_back();
              else
                break;
            }
            ...
            while (!bid_prices.empty()) {
              if (!bid_prices[bid_prices.size() - 1].get_level_size_impl().has_value())
                bid_prices.pop_back();
              else
                break;
            }
