...
template <std::totally_ordered T> struct TreeNode {
  T val;
  TreeNode *left;
  TreeNode *right;
...
template <std::totally_ordered T> class BinarySearchTree {
public:
  ...
  static void inorder_traversal_cb(TreeNode<T> *root,
    ...
    if (root->left != nullptr)
      inorder_traversal_cb(root->left, callback, context);

    if (!callback(context, root->val))
      return;

    if (root->right != nullptr)
      inorder_traversal_cb(root->right, callback, context);
  }
