...
public:
  ...
  std::optional<int> __attribute__((noinline)) get_level_size_impl() const {
    if (orders.empty()) {
      return std::nullopt;
    }
    int orders_size = 0;
  ...
  std::optional<int> __attribute__((noinline)) get_level_price_impl() const {
    if (orders.empty()) {
      return std::nullopt;
    }
