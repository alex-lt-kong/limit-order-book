...
class OrderBookArray : public IOrderBook<OrderBookArray> {
  ...
  but size smaller than target_size, then it iterates from 4000 to 0 for (auto
  &price_level: transform(prices)) { auto level_price_cent =
  price_level.get_level_price(); if (!level_price_cent.has_value()) {
        ++miss;
        if constexpr (!benchmark_performance)
          std::cerr << "prices.size(): " << prices.size() << "hit: " << hit <<
  ", miss: " << miss << ", hit rate: " << hit / (hit + miss) << "\n"; continue;
      }
      ...
      if constexpr (!benchmark_performance)
        std::cerr << "prices.size(): " << prices.size() << "hit: " << hit << ",
  miss: " << miss << ", hit rate: " << hit / (hit + miss) << "\n"; if
  (target_size > level_size.value()) { target_size -= level_size.value();
